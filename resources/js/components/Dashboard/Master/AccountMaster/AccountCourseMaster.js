...
function createData(account_id, date, time, account_name, account_from_customer, account_to_therapist, account_during, account_memo) {
    return {
        account_id,
        date,
        time,
...
function AccountCourseMaster() {
    ...
    const account_during_input = useRef(null);
    const account_memo_input = useRef(null);

    const [ edit_account_id, setEditAccountId ] = useState(0);
    const [update_account_id, setUpdateAccountId] = useState(0);
    ...
    const [edit_account_during, setEditAccountDuring] = useState('');
    ...
    rows = accountCourses.map((item, index)=>{
        return createData(
            item.id, 
    ...
    const jQueryCode = () => {
        $('.btnDeleteAccountCourse').each((index, item) => {
            $(item).click((event)=>{
                ...
                axios
                    .post('/api/deleteAccountCourse', {id: deleteAccountId})
                    .then((res)=>{
        ...
        $('.btnEditAccountCourse').each(( index, item )=>{
            $(item).click((event)=>{
                ...
                let editAccountDuring = event.currentTarget.id.split(':*_*:')[5];
                let editAccountMemo = event.currentTarget.id.split(':*_*:')[6];

                setEditAccountId(editAccountId);
                setUpdateAccountId(editAccountId);
                setEditAccountName(editAccountName);
                setEditAccountFromCustomer(editAccountFrom);
                setEditAccountToTherapist(editAccountTo);
                setEditAccountDuring(editAccountDuring);
    ...
    const handleSubmit = async (event) => {
        ...
        const accountInfo = {
            ...
            account_to: formData.get('account_to_therapist'),
            account_during: formData.get('account_during'),
            account_memo: formData.get('memo')
        };

        await window.axios
            .post('/api/registAccountCourse', accountInfo)
            .then((response) => {
                ...
                account_during_input.current.value = '';
    ...
    const onEditAccount = async (event) => {
        ...
        const accountInfo = {
            ...
            account_updateId: formData.get('update_account_id'),
            account_name: formData.get('edit_account_name'),
            account_from: formData.get('edit_account_from_customer'),
            account_to: formData.get('edit_account_to_therapist'),
            account_during: formData.get('edit_account_during'),
            account_memo: formData.get('edit_memo')
        };

        await window.axios
            .post('/api/editAccountCourse', accountInfo)
            .then((response) => {
            ...
            .catch(error => {
                toast.error('idが重複しています。idをもう一度入力してください。');
            });
    }
                ...
                <Container maxWidth={'md'}>
                    <Box component={"form"} onSubmit={handleSubmit}>
                        ...
                        <Container maxWidth={'sm'}>
                            ...
                            <FormControl variant="standard" fullWidth sx={{ mt: 2 }}>
                                <InputLabel htmlFor="discount">コース時間</InputLabel>
                                <Input
                                    id="account_during"
                                    name='account_during'
                                    type="number"
                                        ...
                                        {stableSort(rows, getComparator(order, orderBy))
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <TableCell align='center'>{row.account_during}</TableCell>
                    ...
                    <Container maxWidth={'md'}>
                        <Box component={'form'} onSubmit={onEditAccount}>
                            <FormControl variant="standard" fullWidth>
                                <InputLabel htmlFor="update_account_id">ID</InputLabel>
                                <Input
                            ...
                            <FormControl variant="standard" fullWidth sx={{ mt: 2 }}>
                                <InputLabel htmlFor="discount">コース時間</InputLabel>
                                <Input
                                    id="edit_account_during"
                                    name='edit_account_during'
                                    type="number"
                                    endAdornment={<InputAdornment position="end">分</InputAdornment>}
                                    inputProps={{
                                    'aria-label': 'コース時間',
                                    ...
                                    label='コース時間'
                                    placeholder="なし"
                                    value={edit_account_during}
                                    onChange={e => setEditAccountDuring(e.target.value)}
                                />
                            </FormControl>
                            <TextareaAutosize
                                id="edit_memo"
...
export default AccountCourseMaster
