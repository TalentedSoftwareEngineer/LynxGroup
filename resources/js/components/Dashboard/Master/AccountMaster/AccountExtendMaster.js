...
function createData(account_id, date, time, account_name, account_from_customer, account_to_therapist, extend_during, account_memo) {
    return {
        account_id,
        date,
        time,
...
function AccountExtendMaster() {
    ...
    const extend_duringinput = useRef(null);
    ...
    const [edit_extend_during, setEditExtendDuring] = useState('');
    ...
    rows = accountExtends.map((item, index)=>{
        return createData(
            item.id, 
    ...
    const jQueryCode = () => {
        $('.btnDeleteAccountCourse').each((index, item) => {
            $(item).click((event)=>{
                ...
                axios
                    .post('/api/deleteAccountExtend', {id: deleteAccountId})
                    .then((res)=>{
        ...
        $('.btnEditAccountCourse').each(( index, item )=>{
            $(item).click((event)=>{
                ...
                let editExtendDuring = event.currentTarget.id.split(':*_*:')[5];
                ...
                setEditExtendDuring(editExtendDuring);
    ...
    const handleSubmit = async (event) => {
        ...
        const accountInfo = {
            ...
            extend_during: formData.get('extend_during'),
            account_memo: formData.get('memo')
        };

        await window.axios
            .post('/api/registAccountExtend', accountInfo)
            .then((response) => {
                ...
                extend_duringinput.current.value = '';
    ...
    const onEditAccount = async (event) => {
        ...
        const accountInfo = {
            ...
            extend_during: formData.get('edit_extend_during'),
            account_memo: formData.get('edit_memo')
        };

        await window.axios
            .post('/api/editAccountExtend', accountInfo)
            .then((response) => {
                ...
                <Container maxWidth={'md'}>
                    <Box component={"form"} onSubmit={handleSubmit}>
                        ...
                        <Container maxWidth={'sm'}>
                            ...
                            <FormControl variant="standard" fullWidth sx={{ mt: 2 }}>
                                <InputLabel htmlFor="extend_during">延長時間</InputLabel>
                                <Input
                                    id="extend_during"
                                    name='extend_during'
                                    type="number"
                                        ...
                                        {stableSort(rows, getComparator(order, orderBy))
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <TableCell align='center'>{row.extend_during}</TableCell>
                ...
                >
                    <Container maxWidth={'md'}>
                        <Box component={'form'} onSubmit={onEditAccount}>
                            ...
                            <FormControl variant="standard" fullWidth sx={{ mt: 2 }}>
                                <InputLabel htmlFor="discount">延長時間</InputLabel>
                                <Input
                                    id="edit_extend_during"
                                    name='edit_extend_during'
                                    type="number"
                                    endAdornment={<InputAdornment position="end">分</InputAdornment>}
                                    inputProps={{
                                    'aria-label': '延長時間',
                                    ...
                                    label='延長時間'
                                    placeholder="なし"
                                    value={edit_extend_during}
                                    onChange={e => setEditExtendDuring(e.target.value)}
                                />
                            </FormControl>
                            <TextareaAutosize
                                id="edit_memo"
...
export default AccountExtendMaster
